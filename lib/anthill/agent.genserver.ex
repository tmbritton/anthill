defmodule Anthill.AgentGenServer do
  use GenServer
  @max_retries 3
  ...
  @impl true
  def handle_cast({:new_message, %Anthill.Message{} = msg}, %{state: current_state} = state)
      when current_state in [:loading, :error, :outputting] do
  ...
  @impl true
  def handle_info(
        {:llm_response, %Anthill.Message{role: :assistant} = msg},
        %{state: :loading} = state
      ) do
